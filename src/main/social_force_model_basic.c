...
/*
  ...
  Parameters: 
        ...
        people_repulsion_term: (2n,2n) : matrix containing the force of repulsion between person i and j
                            n: number of people
*/
void update_people_repulsion_term(float *position, float *desired_direction, float *actual_speed, float *people_repulsion_term, int n)
{
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n; j++)
    {
      if (i == j)
        continue;
      float rx_ab = position[i * 2] - position[j * 2];         //1 add, 1 flop
      float ry_ab = position[i * 2 + 1] - position[j * 2 + 1]; //1 add, 1 flop
      ...
      float delta_b = vb * TIMESTEP; // 1 mul => 1 flops

      float r_ab_norm = sqrt(rx_ab * rx_ab + ry_ab * ry_ab); //(1) 1 sqrt, 1 add, 2 mul => 4 flops

      //me stands for "minus e"
      float rx_ab_mex = rx_ab - delta_b * ex_b; //1 add, 1 mul => 2 flops
      float ry_ab_mey = ry_ab - delta_b * ey_b; //1 add, 1 mul => 2 flops

      float r_ab_me_norm = sqrt(rx_ab_mex * rx_ab_mex + ry_ab_mey * ry_ab_mey); //(2)  1 sqrt, 2 mul, 1 add => 4 flops

      float repulsion_x = rx_ab / r_ab_norm + rx_ab_mex / r_ab_me_norm; //2 divs, 1 add => 3 flops
      float repulsion_y = ry_ab / r_ab_norm + ry_ab_mey / r_ab_me_norm; //2 divs, 1 add => 3 flops

      float b = sqrt((r_ab_norm + r_ab_me_norm) * (r_ab_norm + r_ab_me_norm) - (delta_b * delta_b)) / 2; //1 sqrt, 3 add, 2 mul, 1 div => 7 flops
      ...
      float check = ex_a * (-repulsion_x) + ey_a * (-repulsion_y);                              //2 mult, 1 add => 3 flops
      float threshold = sqrt(repulsion_x * repulsion_x + repulsion_y * repulsion_y) * cos(PSI); //1 sqrt, 2 mults, 1 add => 4 flops
...
void update_border_repulsion_term(float *position, float *borders, float *border_repulsion_term, int n, int n_borders)
{
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n_borders; j++)
    {
      ...
      float r_aB_norm = fabs(ry_aB); // 1 fabs => 1 flop

      float repulsion_x = exp_fast((-r_aB_norm) / R) * (rx_aB / r_aB_norm); //1 exp, 2 div, 1 mult => 3 flops + 1 exp
      repulsion_x *= U_ALPHA_B / R;                                     // 1 mult, 1 div => 2 flops

      float repulsion_y = exp_fast((-r_aB_norm) / R) * (ry_aB / r_aB_norm); //1 exp, 2 div, 1 mult => 3 flops + 1 exp
