...
/*
  ...
  Parameters: 
        ...
        people_repulsion_term: (2n,2n) : matrix containing the force of repulsion between person i and j
                            n: number of people
*/
void update_people_repulsion_term_double(double *position, double *desired_direction, double *actual_speed, double *people_repulsion_term, int n)
{
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n; j++)
                                        ...
                                        double *people_repulsion_term, double *border_repulsion_term, double *social_force, double *desired_speed, double *desired_max_speed)
{
  ...
  {
    // update variables
    update_desired_direction_double(position, final_destination, desired_direction, number_of_people);
    update_acceleration_term_double(desired_direction, acceleration_term, actual_velocity, desired_speed, number_of_people);
    update_people_repulsion_term_double(position, desired_direction, speed, people_repulsion_term, number_of_people);
    update_border_repulsion_term_double(position, borders, border_repulsion_term, number_of_people, N_BORDERS);
    compute_social_force_double(acceleration_term, people_repulsion_term, border_repulsion_term, social_force, number_of_people, N_BORDERS);
    update_position_double(position, desired_direction, speed, social_force, actual_velocity, desired_max_speed, number_of_people);
                                             ...
                                             double *people_repulsion_term, double *border_repulsion_term, double *social_force, double *desired_speed, double *desired_max_speed)
{
  ...
  {
    get_filename();
    output_to_file_initial_state_double(filename_global, position, speed, desired_direction, final_destination, number_of_people, n_timesteps);
  }
  ...
  for (int step = 0; step < ntimesteps; step++)
  {
    ...
    if (arguments.visual)
    {
      output_to_file_persons_double(filename_global, position, speed, desired_direction, final_destination, number_of_people, n_timesteps);
    }
  }
