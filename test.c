...
void control_speed(double *People, double *acceleration_terms, double *desired_velocity, int n);
void update_position(double *People, double *desired_velocity, int n);
void show_people(double *People);
// main function
int main()
{
  printf("Test\n");
  double *People, *borders, *actual_velocity, *acceleration_terms, *desired_velocity;
  printf("Allocate memory\n");
  allocate_memory(&People, (N_FEATURES*NUMBER_OF_PEOPLE));
  allocate_memory(&borders, N_BORDERS);
  allocate_memory(&actual_velocity, 2*NUMBER_OF_PEOPLE);
  allocate_memory(&acceleration_terms, 2*NUMBER_OF_PEOPLE);
  allocate_memory(&desired_velocity, 2*NUMBER_OF_PEOPLE);

  printf("fill vectors\n");
  initialize_people(People, NUMBER_OF_PEOPLE);
  initialize_borders(borders, N_BORDERS);
  initialize_vector_to_zero(desired_velocity, 2*NUMBER_OF_PEOPLE);
  //printf("Walkway width: %f, Walkway length: %f, MAX_RAND: %d\n", WALK_WAY_WIDTH, WALK_WAY_LENGTH, RAND_MAX);
  printf("y-values of borders: ");
  ...
  for(int i = 0; i<N_TIMESTEP;i++)
  {
      ...
      update_acceleration_term(People, acceleration_terms, actual_velocity, NUMBER_OF_PEOPLE);
      
      control_speed(People, acceleration_terms, desired_velocity, NUMBER_OF_PEOPLE);
      update_position(People, desired_velocity, NUMBER_OF_PEOPLE);
  }
  ...
  destroy(People);
  destroy(borders);
  destroy(actual_velocity);
  destroy(acceleration_terms);
  destroy(desired_velocity);
...
void update_position(double *People, double *desired_velocity, int n)
{
    for(int i=0;i<n;i++)
    {
        People[N_FEATURES*i] += desired_velocity[2*i]*TIMESTEP;
        People[N_FEATURES*i + 1] += desired_velocity[2*i+1]*TIMESTEP;
    }
}
...
void control_speed(double *People, double *acceleration_terms, double * desired_velocity, int n)
{
    for(int i = 0; i<n;i++)
    {
        ...
        desired_velocity[2*i] += acceleration_terms[2*i]*TIMESTEP;
        desired_velocity[2*i+1] += acceleration_terms[2*i+1]*TIMESTEP;
        printf("Prefered Velocity of person %d = (%f,%f)\n", i, desired_velocity[2*i], desired_velocity[2*i+1]);
        double norm_value = sqrt(pow(desired_velocity[2*i],2) + pow(desired_velocity[2*i + 1],2));
        printf("Norm value: %f\n", norm_value);
        if(norm_value > MAX_SPEED)
        {
            control_value = MAX_SPEED/norm_value;
            printf("control value updated to %f\n", control_value);
        }
        desired_velocity[2*i]*=control_value;
        desired_velocity[2*i +1]*=control_value;
        printf("Prefered Velocity [scaled] of person %d = (%f,%f)\n", i, desired_velocity[2*i], desired_velocity[2*i+1]);
        People[N_FEATURES*i + 2] = sqrt(pow(desired_velocity[2*i],2) + pow(desired_velocity[2*i + 1],2));
    }
}
...
void show_people(double *People)
{
  ...
  {
      printf("Person %d: position=(%f, %f), speed=%f, direction=(%f, %f), final=(%f, %f)\n", i, People[N_FEATURES*i], People[N_FEATURES*i+1], People[N_FEATURES*i+2], People[N_FEATURES*i+3], People[N_FEATURES*i+4], People[N_FEATURES*i+5], People[N_FEATURES*i+6]);
  }
  printf("\n");
}
...
/*
  ...
  Parameters:   People: array of people
        repulsion_term: matrix containing the force of repulsion between a and b
                     n: number of people
*/
void update_repulsion_term(double *People, double *repulsion_term, int n)
{
